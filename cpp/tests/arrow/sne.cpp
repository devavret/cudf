...
std::unique_ptr<cudf::column> column_from_array_span(
    arrow::ArraySpan const& arr) {
  switch (arr.type->id()) {
    case arrow::Type::STRING: {
      ...
      auto offset_span = cudf::host_span<int32_t const>{
          reinterpret_cast<int32_t*>(arr.buffers[1].data),
          arr.buffers[1].size / sizeof(int32_t)};

      auto d_off = cudf::detail::make_device_uvector_async(
          offset_span, rmm::cuda_stream_default);

      auto chars_span = cudf::host_span<char const>{
          reinterpret_cast<char*>(arr.buffers[2].data),
          static_cast<size_t>(arr.buffers[2].size)};

      auto d_chars = cudf::detail::make_device_uvector_async(
          chars_span, rmm::cuda_stream_default);

      return cudf::make_strings_column(arr.length, std::move(d_off),
                                       std::move(d_chars), std::move(nullmask));
    }
    default:
      CUDF_FAIL("unsupported");
  }
}
...
struct StringEqual {
  static Status ArrayArray(KernelContext* ctx,
    ...
    auto result =
        cudf::binary_operation(*col0, *col1, cudf::binary_operator::EQUAL,
    ...
    cudaMemcpy(out->array_span()->buffers[1].data, result_bits.first->data(),
               result_bits.first->size(), cudaMemcpyDefault);

    return st;
  }

  static Status Exec(KernelContext* ctx,
...
std::shared_ptr<ScalarFunction> MakeCompareFunction(std::string name,
                                                    FunctionDoc doc) {
  auto func = std::make_shared<CompareGPUFunction>(name, Arity::Binary(),
                                                   std::move(doc));
...
arrow::Status Execute() {
  ...
  ARROW_RETURN_NOT_OK(int_builder.Append(10));
  ARROW_RETURN_NOT_OK(int_builder.Append(10));
  ARROW_ASSIGN_OR_RAISE(std::shared_ptr<arrow::Array> int_array,
                        int_builder.Finish());
  ...
  ARROW_RETURN_NOT_OK(str_builder.Append("dog"));
  ARROW_RETURN_NOT_OK(str_builder.Append("dog"));
  ARROW_ASSIGN_OR_RAISE(std::shared_ptr<arrow::Array> str_array,
                        str_builder.Finish());
  ...
  ARROW_RETURN_NOT_OK(str_builder_2.Append("dog"));
  ARROW_RETURN_NOT_OK(str_builder_2.Append("dog"));
  ARROW_ASSIGN_OR_RAISE(std::shared_ptr<arrow::Array> str_array_2,
                        str_builder_2.Finish());

  std::vector<std::shared_ptr<arrow::Field>> schema_vector = {
      arrow::field("int", arrow::int32()), arrow::field("str", arrow::utf8()),
  ...
  auto table_source_options =
      arrow::compute::TableSourceNodeOptions{table, max_batch_size};
  arrow::AsyncGenerator<arrow::util::optional<arrow::compute::ExecBatch>>
      sink_gen;

  // source node
  ARROW_ASSIGN_OR_RAISE(arrow::compute::ExecNode * source_node,
                        arrow::compute::MakeExecNode("table_source", plan.get(),
                                                     {}, table_source_options));

  DCHECK_OK(arrow::compute::GetFunctionRegistry()->AddFunction(
      arrow::compute::MakeCompareFunction("equal", arrow::compute::equal_doc2),
      true));

  // filter node
  arrow::compute::ExecNode* filter_node;
  ARROW_ASSIGN_OR_RAISE(
      filter_node, arrow::compute::MakeExecNode(
                       "filter", plan.get(), {source_node},
                       arrow::compute::FilterNodeOptions{arrow::compute::equal(
                           arrow::compute::field_ref("str"),
                           arrow::compute::field_ref("str2"))}));

  // sink node
  ARROW_RETURN_NOT_OK(
      arrow::compute::MakeExecNode("sink", plan.get(), {filter_node},
                                   arrow::compute::SinkNodeOptions{&sink_gen}));

  auto result = ExecutePlanAndCollectAsTable(
      exec_context, plan, filter_node->output_schema(), sink_gen);
